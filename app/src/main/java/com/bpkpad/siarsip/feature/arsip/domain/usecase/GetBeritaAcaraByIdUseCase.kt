...
class GetBeritaAcaraByIdUseCase @Inject constructor(
    private val repository: ArsipRepository
) {
    operator fun invoke(id: String): Flow<BeritaAcaraItem?> {
        return combine(
            repository.getBeritaAcaraById(id),
            repository.getAllProposals()
        ) { ba, proposals ->
            if (ba == null) return@combine null
